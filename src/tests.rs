...
fn test_pathbuf_leak() {
    ...
    let leaked = buf.leak();
    assert_eq!(leaked.as_str(), "/have/a/cake");
    unsafe {
        drop(String::from_raw_parts(
            leaked.as_mut_str() as *mut str as _,
            len,
            cap,
        ))
    }
...
pub fn test_decompositions_windows() {
    ...
    /**
     * @quirks prefer think paths that not have verbatim prefix and start with / be *nix-like root paths
     */
    t!("/",
    iter: ["/"],
    has_root: true,
    ...
    /**
     * @quirks prefer think paths that not have verbatim prefix and start with \\ be *windows-like root paths
     */
    t!("\\",
    iter: ["\\"],
    has_root: true,
    ...
    /**
     * @quirks think paths start with drive as windows drive paths, and if not contains / sep then default use \ sep
     */
    t!("c:",
    iter: ["c:"],
    has_root: false,
    ...
    /**
     * @quirks think paths start with drive as windows absolute paths, and if contains / then main sep is /
     */
    t!("c:/",
    iter: ["c:", "/"],
    has_root: true,
    ...
    /**
     * @quirks prefer think paths not contains prefix and starts as \ as windows absolute path
     */
    t!("\\\\server",
    iter: ["\\", "server"],
    has_root: true,
    ...
    /**
     * @quirks prefer think paths not verbatim keep slash directions
     */
    t!("\\\\.\\foo/bar",
    iter: ["\\\\.\\foo", "/", "bar"],
    has_root: true,
...
fn test_components_debug() {
    ...
    let expected = "Components([RootDir(Some(\"/\")), Normal(\"tmp\")])";
...
fn test_clone_into() {
    ...
    assert!(path_buf.into_string().capacity() >= 15);
}
...
fn test_unix_absolute() {
    ...
    let current_dir = || Ok(PathBuf::from(r"/workspace"));

    assert!(absolute("", current_dir).is_err());

    let relative = "a/b";
    let mut expected: PathBuf = current_dir().unwrap();
    expected.push(relative);
    assert_eq!(
        absolute(relative, current_dir).unwrap().as_str(),
        expected.as_str()
    );

    // Test how components are collected.
    assert_eq!(
        absolute("/a/b/c", current_dir).unwrap().as_str(),
    ...
    let curdir: PathBuf = current_dir().unwrap();
    assert_eq!(
        absolute("./a", current_dir).unwrap().as_str(),
        curdir.join("a").as_str()
    );
    assert_eq!(
        absolute("../a", current_dir).unwrap().as_str(),
        curdir.join("../a").as_str()
...
fn test_windows_absolute() {
    ...
    let current_dir = || Ok(PathBuf::from(r"D:\workspace"));
    ...
    macro_rules! unchanged(
        ($path:expr) => {
            assert_eq!(absolute($path, current_dir).unwrap().as_str(), Path::new($path).as_str());
        }
    );
    ...
    /**
     * @quirks lack of extra information, think COM1 as relative path
     */
    assert_eq!(
        absolute(r"COM1", current_dir).unwrap().as_str(),
        Path::new(r"D:\workspace\COM1").as_str()
    );

    /**
     * @quirks not trim trailing dot at windows platform
     */
    assert_ne!(
        absolute(r"C:\path..\to.\file.", current_dir)
            .unwrap()
            .as_str(),
        Path::new(r"C:\path..\to\file").as_str()
    );
}
...
#[test]
#[should_panic = "path separator"]
fn test_extension_path_sep_alternate_windows() {
    let mut path = PathBuf::from("path/to/file");
    path.set_extension("d\\test");
